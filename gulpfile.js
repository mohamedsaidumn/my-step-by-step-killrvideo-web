...
gulp.task("images", ["clean.images"], function () {
  return gulp.src(paths.IMAGE_FILES).pipe(gulp.dest(paths.IMAGE_OUTPUT));
});

// Sync new images to build output directory
gulp.task("images.sync", function () {
  return gulp
    .src(paths.IMAGE_FILES)
...
gulp.task("watch", ["server"], function () {
  ...
  plugins.watch(paths.CSS_FILES, function () {
    gulp.start("css");
  });

  // Watch for new images and add them
  plugins.watch(paths.IMAGE_FILES, function () {
    gulp.start("images.sync");
  });

  // Watch for changes to html file and reload
  plugins.watch("server.html", function () {
